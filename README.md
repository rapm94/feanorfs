...
**Background sync (Dropbox-like):** your uncommitted work follows you between machines. `feanorfs start` performs the first sync, installs one background supervisor service, and returns; sync restarts automatically at login.
...
- **One verb onboarding** — on the first computer, `feanorfs start [folder]` creates a secure private hub; on another computer it securely pairs on-LAN (`fnp1-…`), through an off-LAN rendezvous (`fnp2-…`), or joins (`fnr1-…`). The same command syncs, installs the automatic supervisor service, and returns.
...
- **Native lifecycle** — one supervisor login job owns the private hub, every workspace watcher, and the desktop tray at login through launchd on macOS, a user service manager on Linux, or Task Scheduler on Windows; Windows additionally keeps a separate interactive tray task because UI sessions require it. No `nohup`, PID files, or manual reboot setup.
...
- **Desktop tray** — macOS, Linux, and Windows product installers include the same thin tray; `start` registers it at login while the native supervisor service remains the only sync owner.
...
- **Continuous active agents (no Git, no push/pull)** — `feanorfs agent run <name> -- <command>` activates continuous reconciliation for the command's lifetime (configured runners activate it for their agent): saved changes land automatically after a quiet burst, untouched remote paths refresh automatically, overlapping edits pause as explicit conflicts, and signal-linked feedback wakes promptly. No `sync`/`push`/`pull`/`land`/`refresh` commands, no Git repository, no content merging; `spawn` alone and upgrades never activate dormant agents. See [usage](docs/usage.md#continuous-active-agents) and [agent communication](docs/agent-communication.md#continuous-wakeup-active-agents).
