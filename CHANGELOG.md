...
- Wait for the LAN pairing service to be announced before showing its one-time tray/CLI code, bound automatic-hub readiness probes so slow network attempts cannot hide startup failures or outlive the user-facing deadline, preserve native Windows SQLite paths without URL parsing, document hosted-macOS multicast limitations without weakening non-hosted LAN product smoke, and keep Windows product smoke state out of PowerShell's read-only automatic variables.
