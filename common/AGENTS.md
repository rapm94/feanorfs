...
- `mesh_contract.rs` — signed capability-v2 mesh wire types (`NodeId`, bounded ≤16 candidate list with `lan|direct|mapped|reflexive` kinds over `tcp|quic`, canonical `MeshConfig`). Reflexive candidates require QUIC; duplicates are rejected at decode. Decisions: [docs/mesh-transport.md](../docs/mesh-transport.md).
- `invite.rs` carries additive optional `mesh` on workspace/hub capabilities; absence stays valid forever.
- `tray_contract.rs` — bounded secret-free desktop projections. `WorkerStatusSnapshot.mesh_reachability` additively projects the last observed mesh path class (`lan|direct|direct_mapped|punched|unreachable`); `TrayOverviewResult` additively wraps the stable `TrayStatusResult` plus an optional recent-workspace registry so one refresh needs one CLI process; fixtures are snapshot-tested by the client.
...
- `pack_bytes` / `unpack_bytes` — ChaCha20-Poly1305 for new blobs; format v2 and v3 workspaces reject non-AEAD blobs. Unmigrated v1 workspaces fall back to legacy `crypt_bytes` XOR when the AEAD prefix is absent, or when authentication fails AND the caller-expected plaintext size matches the blob length (a valid legacy ciphertext has a 1/256 prefix collision, so its length equals its plaintext; an AEAD-shaped blob failing auth is reported instead of degrading — see `unpack_bytes_checked`); removal requires separately approved representative field evidence.
