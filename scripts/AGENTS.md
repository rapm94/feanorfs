...
- `smoke-windows-product.ps1 -HeadlessRunner` still requires the supervisor, hub, and workspace watcher to run and validates the tray task's exact action plus `InteractiveToken` principal, but accepts the tray task settling `Ready` when no interactive desktop exists. Use this mode only for headless CI gates; omit it for the supplemental interactive tray runtime proof.
