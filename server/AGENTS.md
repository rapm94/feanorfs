...
- Automatic TLS leaves include `feanorfs-<CA fingerprint>.local`; mDNS advertises that CA-bound hostname with address auto-tracking so interface and DHCP changes do not require leaf regeneration or fixed router leases. Custom certificate deployments retain ownership of their DNS names.
