...
**Result:** **NOT fully defended against.**
- **Ciphertext tampering**: The encryption scheme is an unauthenticated XOR stream (Blake3 XOF). There is no MAC/AEAD. An attacker who flips a bit in a ciphertext blob will flip the corresponding bit in the decrypted plaintext. The client verifies the **encrypted** hash on upload (server-side), but on download, the client trusts the server-provided hash. If the server lies about the hash, tampered ciphertext will be accepted. **Do not use FeanorFS against an untrusted server.**
...
- `encrypted_hash = blake3(ciphertext_bytes)` — the CAS blob key. Sent to the server. Used to verify upload integrity.
...
| No server authentication | High | Run on trusted network; future: token/mTLS auth | Known limitation |
...
4. **Server authentication** — Add a shared-secret token header (`Authorization: Bearer ...`) to all API requests. Generated by `feanorfs init` and stored in `config.json`.
5. **Path obfuscation** — Encrypt file paths in the metadata using the same password-derived key, so the server only sees opaque path identifiers.
