...
- Internal modules:
  ...
  - `integrator.rs` — randomized integrator assignment: dispatcher state machine, `orchestrator/integrator-state.json` crash-safe persistence, `ffint1` publish/consume, pre-acceptance timeout/fallback, revocation, cursor-reset fail-closed, and read-only cross-machine conflict materialization. Canonical types/ranking in `feanorfs_common::integrator_contract`; `lock.rs::DispatcherLock` enforces the single-dispatcher invariant.
  ...
  - `upload_registry.rs` — durable per-workspace set of object hashes the hub accepted, used to skip redundant uploads.
  - `paths.rs` — global agent worktrees, conflicts dir, name validation (breaks agent↔conflicts cycle).
...
- Downloads guard against clobbering a file edited between scan and download by comparing the current disk fingerprint against the scan-time disk fingerprint (cache-entry mtime/size), never the server mtime; lazy placeholders are always refreshed. `upload_registry.rs` may skip tree/snapshot/chunk/undo uploads only for hashes covered by a reachability manifest the hub accepted (head closures are never GC-expired); a rejected manifest clears the registry and the sync pass retries once with forced upload of every live file.
