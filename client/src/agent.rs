...
use feanorfs_common::{
    conflict_candidate_paths, detect_concurrent_edits, normalize_path, AgentCheckResult,
...
struct AgentDiff {
    ...
    base_map: HashMap<String, FileState>,
}
...
async fn compute_agent_diff(ctx: &SyncCtx<'_>, name: &str) -> Result<AgentDiff> {
    ...
    if snapshot.is_empty() {
        bail!(
            "No snapshot rows for agent '{}'. Run `feanorfs agent clean {}` then `feanorfs agent spawn {}`.",
            name,
            name,
            name
        );
    ...
    let empty_response = SyncResponse {
...
async fn spawn_agent_with_ctx(
    ...
    if let Err(e) = ctx.db.record_agent_snapshot(&snapshot_entries).await {
...
async fn land_agent_with_ctx(
    ...
    for change in &diff.our_changes {
        ...
        if main_path.exists() && !change.deleted {
            ...
            let mtime = file_mtime_ms(&main_path).await?;
    ...
    if !landed_states.is_empty() {
        ...
        let _ = crate::commands::process_uploads(ctx, &upload_response, &local_after).await?;
...
pub async fn refresh_agent(
    ...
    for path in &diff.conflict_risk {
        if let Some(theirs) = diff.their_changes.iter().find(|f| &f.path == path) {
            ...
            let _ =
                crate::commands::process_downloads(&agent_ctx, &response, &local, false).await?;
            if let Some(base_entry) = diff.base_map.get(path) {
                db.update_agent_snapshot_base(name, path, base_entry)
                    .await?;
            }
...
/// Back-compat alias for `check_agent`.
pub async fn commit_agent(
    ...
    let check = check_agent(base, db, api, workspace_id, name, password).await?;
    Ok(feanorfs_common::AgentCommitResult {
        agent_name: check.agent_name,
        our_changes: check.our_changes,
        their_changes: check.their_changes,
...
fn write_proposal_if_clean(conflict_dir: &Path, edit: &mut ConcurrentEdit) -> Result<()> {
    ...
    let ob = std::fs::read(&original).unwrap_or_default();
