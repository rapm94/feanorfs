...
- `src/main.rs` — event-loop wiring only (menu rebuild, watch child process).
- `src/model.rs` — state and derived view model.
- `src/menu.rs` — pure menu construction and action IDs (no native windows).
- `src/actions.rs` — action dispatch and background task policy.
- `src/dialogs.rs` — native dialogs and copy, including the typed setup-stage classification.
- `src/feanorfs.rs` — bounded typed CLI subprocess adapter (`CapturedCommand`: exact `OsString` args, bounded stdout/stderr, timeout/cancellation, typed JSON decode, one-line secret stdin) plus the explicit pair/join state machines.
...
- Background sync belongs to the single supervisor service (`com.feanorfs.agent`) installed by `feanorfs start`; it owns the private hub, every workspace watcher, and the desktop tray on macOS/Linux. The tray detects that managed watcher, stops/restarts it through the supervisor around Keep/Land/Sync Now, and refuses only unmanaged terminal watchers. It may spawn a legacy `feanorfs sync` child for workspaces without an installed service.
...
- Every normal tray launch holds the exclusive per-user `~/.feanorfs/tray-instance.lock` for its complete process lifetime. Tray ownership deliberately ignores `FEANORFS_HOME`, so isolated data profiles cannot create multiple desktop trays for one signed-in user. A manual secondary launch from any current install path exits successfully before native UI initialization. A `--managed` secondary exits with a retryable nonzero status so the macOS/Linux supervisor or Windows Task Scheduler adopts ownership after an external crash. A **Quit** from an external primary writes a one-shot protected marker before releasing the lock; the next managed attempt consumes it and exits cleanly. A managed primary's clean exit is observed directly. Both paths preserve the user's choice without confusing a crash with a Quit. `--version` exits before claiming the user lock, and runner-test profiles may do so only in debug builds.
...
- A non-empty tray join is a two-stage bounded child protocol: the CLI completes PAKE, retains the decrypted invite, and emits only typed counts plus capped path examples. The tray shows upload/download/identical/conflict consequences and the ignore-policy replacement warning, then sends public `CONFIRM`/`CANCEL` on the existing stdin pipe. Cancel is not shown as setup failure; the CLI re-previews immediately before mutation.
- Setup completion copy distinguishes pairing, initial sync, supervisor workspace registration, and tray registration. Partial failure must describe completed durable work accurately and route retry through `start -- <folder>`; never claim a configured or already-synced folder was not added or unchanged.
- The conflicts submenu offers strongly confirmed bulk local and bulk mirror choices with the exact pending count and overwrite/deletion consequences. Both delegate to one CLI bulk operation followed by sync; the tray never loops per path or merges content.
...
- Target-specific crate features are mandatory: GTK/AppIndicator plus XDG portal/Wayland support on Linux, common-controls-v6 on Windows, and native AppKit on macOS. Do not enable muda's optional `libxdo`: FeanorFS has no Copy/Cut/Paste/SelectAll predefined menu actions, and removing that unused feature avoids incompatible `libxdo.so.3`/`.so.4` release ABIs across Debian and Arch/CachyOS. Clipboard access is text-only and clears a pairing code only when the clipboard still contains that exact code.
