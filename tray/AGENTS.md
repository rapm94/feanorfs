...
- With no workspace, the tray stays alive and offers **Start Mirroring a Folder…** through `rfd`'s native picker. A verified installer may pass only the public `--first-run` hint; when no configured current/recent workspace resolves, present one native three-way choice: **Start Mirroring a Folder…**, **Join Another Computer…**, or **Not Now**. Route the first two into the exact existing menu actions; the choice itself receives no capability or secret. Existing workspaces, login launches, unknown arguments, and **Not Now** must not re-prompt or change state. Setup shells `feanorfs start -- <folder>` with captured output, so secrets remain hidden and the tray never duplicates onboarding or sync logic. The same action becomes **Add Another Folder…** after setup.
